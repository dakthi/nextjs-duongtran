  ...
  callbacks: {
    ...
    async session({ session, token }) {
      ...
      if (session?.user && token) {
        const tokenWithFields = token as {
          id?: string
          role?: string
          email?: string | null
          name?: string | null
        ...
        if (typeof tokenWithFields.id === "string") {
          (session.user as any).id = tokenWithFields.id
        }

        if (typeof tokenWithFields.role === "string") {
          (session.user as any).role = tokenWithFields.role
        }

        if (typeof tokenWithFields.email === "string") {
          session.user.email = tokenWithFields.email
        }

        if (tokenWithFields.name !== undefined) {
          session.user.name = tokenWithFields.name ?? undefined
        }
      }
      return session
    },
